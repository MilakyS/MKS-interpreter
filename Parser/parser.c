...
static char *mks_strndup(const char *s, size_t n) {
    size_t len = 0;
    while (len < n && s[len]) {
        len++;
    }
...
ASTNode* parser_parse_factor(Parser *parser) {
    ...
    if (parser->current_token.type == TOKEN_TYPE_NUMBER) {
        ...
        char *str = mks_strndup(parser->current_token.start, parser->current_token.length);
        parser_eat(parser, TOKEN_TYPE_STRING);
        node = create_ast_string(str, line);
    }
    else if (parser->current_token.type == TOKEN_IDENTIFIER) {
        char *name = mks_strndup(parser->current_token.start, parser->current_token.length);
    ...
    while (parser->current_token.type == TOKEN_LBRACKET || parser->current_token.type == TOKEN_DOT) {
        if (parser->current_token.type == TOKEN_LBRACKET) {
            ...
            char *m_name = mks_strndup(parser->current_token.start, parser->current_token.length);
...
ASTNode* parser_parse_statement(Parser *parser) {
    ...
    if (type == TOKEN_KW_USING) {
        ...
        char *path = mks_strndup(parser->current_token.start, parser->current_token.length);
        parser_eat(parser, TOKEN_TYPE_STRING);
        char *alias = NULL;
        if (parser->current_token.type == TOKEN_IDENTIFIER) {
            if (parser->current_token.length == 2 && strncmp(parser->current_token.start, "as", 2) == 0) {
                parser_advance(parser);
                alias = mks_strndup(parser->current_token.start, parser->current_token.length);
                parser_eat(parser, TOKEN_IDENTIFIER);
            }
        }
    ...
    if (type == TOKEN_KW_FOR) {
        ...
        if (parser->current_token.type != TOKEN_SEMICOLON) {
            if (parser->current_token.type == TOKEN_KW_VAR) {
                ...
                parser_eat(parser, TOKEN_KW_VAR);
                char *v_name = mks_strndup(parser->current_token.start, parser->current_token.length);
    ...
    if (type == TOKEN_KW_FNC) {
        ...
        char *f_name = mks_strndup(parser->current_token.start, parser->current_token.length);
        parser_eat(parser, TOKEN_IDENTIFIER);
        parser_eat(parser, TOKEN_LPAREL);
        ...
        if (parser->current_token.type != TOKEN_RPAREL) {
            do {
                ...
                params[p_count++] = mks_strndup(parser->current_token.start, parser->current_token.length);
                parser_eat(parser, TOKEN_IDENTIFIER);
            } while (parser->current_token.type == TOKEN_COMMA);
        }
        parser_eat(parser, TOKEN_RPAREL);
